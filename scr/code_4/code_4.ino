...
int obs_number[4] = {0x60,0x61,0x62,0x63};
...
void setup()
{
  ...
  pinMode(SWITCH , INPUT);//模式输入引脚
  pinMode(PPM_INT, INPUT);//PPM输入引脚
  attachInterrupt(PPM_INT, PPM, CHANGE);
...
void PPM()
{
  if (digitalRead(PPM_INT) == HIGH)
    ...
    PPM_ASS = ++PPM_ASS;
    if (PPM_ASS == 8) {PPM_ASS = 1;}
    if (PPM_HOEL_TIME1 > 3000) { PPM_ASS = 7;}
    if (PPM_ASS == 1) {RC_[1] = PPM_HOEL_TIME1;}
    if (PPM_ASS == 2) {RC_[2] = PPM_HOEL_TIME1;}
    if (PPM_ASS == 3) {RC_[3] = PPM_HOEL_TIME1;}
    if (PPM_ASS == 4) {RC_[4] = PPM_HOEL_TIME1;}
    if (PPM_ASS == 5) {RC_[5] = PPM_HOEL_TIME1;}
    if (PPM_ASS == 6) {RC_[6] = PPM_HOEL_TIME1;}
  }
}
int sitwch()
{ int ADC_A0 = analogRead(SWITCH);if (ADC_A0 < 512){return 0;}else {return 1;}}
